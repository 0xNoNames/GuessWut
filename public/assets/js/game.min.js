...
$(function () {
    ...
    socket.on('update', (socket_msg) => {
        ...
        for (let i = 0; i < score_array.length; i++) {
            $('#score_div').append('<ul><span>' + score_array[i][0] + '</span><span>' +
                score_array[i][1] + '</span></ul>');
        }
    });
    ...
    socket.on('chat', (socket_chat) => {
        ...
        if (chat_json.win) {
            if (chat_json.me) {
                $('#chat_div').append('<div id="chat_user_win"><span id="chat_user">' + chat_json.user + '</span> (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            } else {
                $('#chat_div').append('<div class="chat_win">' + chat_json.user + '</span> (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            }
        } else if (chat_color) {
            if (chat_json.me) {
                $('#chat_div').append('<div class="chat_grey"><span id="chat_user">' + chat_json.user + '</span> (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            } else {
                $('#chat_div').append('<div class="chat_grey">' + chat_json.user + ' (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            }
            chat_color = !chat_color;
        } else {
            if (chat_json.win) {
                $('#chat_div').append('<div class="chat_win"><span id="chat_user">' + chat_json.user + '</span> (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            } else if (chat_json.me) {
                $('#chat_div').append('<div class="chat_black"><span id="chat_user">' + chat_json.user + '</span> (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            } else {
                $('#chat_div').append('<div class="chat_black">' + chat_json.user + ' (' + chat_json.time + 's): ' + chat_json.chat_msg + '</div>');
            }
            chat_color = !chat_color;
        }
        $('#chat_div').animate({
